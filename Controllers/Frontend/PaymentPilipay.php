...
class Shopware_Controllers_Frontend_PaymentPilipay extends Shopware_Controllers_Frontend_Payment
{
    ...
    public function init()
    {
        ...
        $this->basePath     = Shopware()->Shop()->getHost() . Shopware()->Shop()->getBasePath() . Shopware()
                ->Shop()
                ->getBaseUrl();
    ...
    public function paymentAction()
    {
        ...
        $warehouse = Shopware()
            ->Models()
            ->getRepository('Shopware\CustomModels\PilipayWarehouses\PilipayWarehouses')
        ...
        $country = Shopware()
            ...
            ->findOneBy(array('iso' => $warehouse->getCountryIsoCode()));
        if ( ! $country) {
...
//                'price' => (string) $value['additional_details']['price'] * 100,
                ...
                'weight'     => (string)intval(str_replace(',', '.',
                        (isset($value['additional_details']['weight']) ? $value['additional_details']['weight'] : 0)) * 1000)
            ));
